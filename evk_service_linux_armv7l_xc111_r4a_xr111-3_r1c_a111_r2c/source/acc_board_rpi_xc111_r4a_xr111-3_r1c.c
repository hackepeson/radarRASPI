...
acc_status_t acc_board_gpio_init(void)
{
	...
	if (
		...
		(status = acc_device_gpio_set_initial_pull(GPIO1_PIN, 0)) ||
		(status = acc_device_gpio_set_initial_pull(GPIO2_PIN, 0)) ||
		(status = acc_device_gpio_set_initial_pull(GPIO3_PIN, 0)) ||
		(status = acc_device_gpio_set_initial_pull(RSTn_PIN, 1)) ||
		(status = acc_device_gpio_set_initial_pull(ENABLE_PIN, 0)) ||
		(status = acc_device_gpio_set_initial_pull(PMU_ENABLE_PIN, 0)) ||
		(status = acc_device_gpio_set_initial_pull(GLOREF_EN_PIN, 0)) ||
		(status = acc_device_gpio_set_initial_pull(CE_A_PIN, 0)) ||
		(status = acc_device_gpio_set_initial_pull(CE_B_PIN, 0))
	) {
	...
	if (
		...
		(status = acc_device_gpio_input(GPIO1_PIN)) ||
		(status = acc_device_gpio_input(GPIO2_PIN)) ||
		(status = acc_device_gpio_input(GPIO3_PIN)) ||
		(status = acc_device_gpio_write(RSTn_PIN, 0)) ||
		(status = acc_device_gpio_write(ENABLE_PIN, 0)) ||
		(status = acc_device_gpio_write(PMU_ENABLE_PIN, 0)) ||
		(status = acc_device_gpio_write(GLOREF_EN_PIN, 0)) ||
		(status = acc_device_gpio_write(CE_A_PIN, 0)) ||
		(status = acc_device_gpio_write(CE_B_PIN, 0))
	) {
...
acc_status_t acc_board_start_sensor(acc_sensor_t sensor)
{
	...
	if (acc_board_all_sensors_inactive()) {
		...
		status = acc_device_gpio_write(PMU_ENABLE_PIN, 1);
		if (status != ACC_STATUS_SUCCESS) {
